...
## Discussion 1: GBM Poor Performance
...
## Discussion 2: Important Variables

While the random forest model(best performing model) delivered underwhelming predictive performance, its feature importance outputs revealed intriguing insights. In recent years, particularly amid post-pandemic inflation concerns, Federal Reserve Chairman Jerome Powell has repeatedly emphasized the Fed’s dual mandate: achieving maximum employment and maintaining stable inflation (targeted at 2%). The Fed employs monetary policy tools—such as adjusting interest rates and controlling the money supply—to keep the economy balanced, either by injecting liquidity during downturns or tightening to cool overheated conditions. Remarkably, even without finance-specific preprocessing, the TF-IDF method identified economic development and unemployment rate as the most significant variables in the FOMC statements. This alignment with the Fed’s stated priorities highlights a key lesson from this project: TF-IDF can effectively extract meaningful signals from text data when the corpus has a consistent structure.

Reflecting on how TF-IDF processes the data, I learned that its success on identifying releveant important features(words) in this project likely stems from the Fed’s consistent communication style. FOMC statements appear to follow a standardized template, with the majority of the text remaining uniform across releases. Only a small portion—typically words or sentences tied to the economic outlook, employment, or inflation—changes to reflect current conditions. TF-IDF’s inverse log weighting penalizes frequently occurring terms, assigning higher importance to rarer, more distinctive words. This mechanism allowed the preprocessing method to spotlight the Fed’s subtle language shifts, which are critical for understanding economic policy and predicting market responses. From this, I realized that TF-IDF thrives when the text corpus is uniform, as it can isolate and emphasize unique, high-impact terms.

This experience taught me a critical requirement for applying TF-IDF in text-based modeling: uniformity across the corpus is essential. The consistent structure of the FOMC statements enabled TF-IDF to capture key differences effectively. However, if I were to expand this approach to include multiple document sources with varying styles or structures, a new challenge would arise. To address this, I learned that a preliminary step—grouping similar documents before preprocessing—would be necessary. By clustering texts with comparable formats or themes, I could preserve the uniformity that TF-IDF relies on, maximizing its ability to extract meaningful features and improve model performance.

# Limitation

The concept of "priced in" could be a reason for the poor predictive power. This term describes how market participants form expectations about the Fed’s actions—drawing from prior committee comments and historical decisions—and adjust asset prices accordingly before the official announcement is made. As a result, by the time the Fed releases its statement, much of the market’s reaction has already occurred, reducing the announcement’s immediate impact on prices. This anticipatory behavior is encapsulated in the popular phrase "buy the rumor, sell the news," which highlights how traders act on speculation and then adjust positions once the news is confirmed. To address this issue, additional data like 10-year US gov't bond price, VIX, and prior stock trend can be reflected in the model. 

## Conclusion
This project investigates the potential of leveraging Federal Open Market Committee (FOMC) statements to forecast one-day returns of the NASDAQ-100 ETF (QQQ) using advanced machine learning techniques, including ridge regression, random forest, and gradient boosting machine (GBM) models. The textual data from the FOMC statements was preprocessed by eliminating stop words and irrelevant terms, then transformed into numerical features via TF-IDF (Term Frequency-Inverse Document Frequency) methodology. Among the evaluated models, the random forest approach demonstrated the strongest performance, achieving a test root mean squared error (RMSE) of 0.48%. However, when contextualized against QQQ's average daily true range of approximately 1%, this error constitutes nearly half of the typical daily volatility, suggesting that the model’s predictive accuracy is insufficient for practical investment applications. Interestingly, the random forest model identified terms associated with unemployment and interest rates as significant predictors, implying that the Federal Reserve’s commentary on these topics may exert a measurable influence on short-term market dynamics. Nevertheless, the model’s practical effectiveness is hindered by its relatively high error margin compared to daily market fluctuations, limiting its current utility. 

However' one interesting finding is the effectivness of TF-IDF proprocessing on identifying key terms as the most important features are unemployment rate and economic development which are related to the Fed's dual mandate goals.
